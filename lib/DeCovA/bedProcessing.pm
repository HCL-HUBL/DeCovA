...
#					if (exists $intervals{$chr}{$start}{$end}{"NM"}{$nm}{"exon"}) { $someExon{${$RefID}{$nm}{"gene"}} = 1; }  # this not to print introns of nms if some other nms with exons, in same gene
				...
				foreach my$gene (sort(keys%genes)) {
					if (exists ${$reAnnot_r}{"item"}{"g"}) {
						$geneTxt[$g] = "$gene";
						if ( exists ${$reAnnot_r}{"item"}{"t"} || exists ${$reAnnot_r}{"item"}{"i"} || exists ${$reAnnot_r}{"item"}{"e"} ) { $geneTxt[$g] .= ":"; }
						}
					my@nmTxt = (); my$n = 0;
					foreach my$nm (sort@{ $genes{$gene} }) {
#						if (exists $intervals{$chr}{$start}{$end}{"NM"}{$nm}{"exon"} || (exists $intervals{$chr}{$start}{$end}{"NM"}{$nm}{"intron"} && !exists $someExon{$gene}) ) {
							if (exists ${$reAnnot_r}{"item"}{"t"}) {
								$nmTxt[$n] = "$nm";
								if ((exists $intervals{$chr}{$start}{$end}{"NM"}{$nm}{"intron"} && exists ${$reAnnot_r}{"item"}{"i"}) || ($intervals{$chr}{$start}{$end}{"NM"}{$nm}{"exon"} && exists ${$reAnnot_r}{"item"}{"e"}) ) { $nmTxt[$n] .= ":"; }
								}
...
while ( ($i<($N_exons-1)) && ($bedstart > ${$RefID_r}{$nm}{"ex_ends"}[$i]) ) {
	$i++;
	}
if ($bedEnd < ${$RefID_r}{$nm}{"ex_starts"}[$i]) {
	...
	while ( ($i<($N_exons-1)) && ($bedEnd >= ${$RefID_r}{$nm}{"ex_starts"}[$i]) ) {
		...
		else { ${$interval_r}{$chr}{$bedstart}{$bedEnd}{"NM"}{$nm}{"NC"} = 1; }
		$i++
		}
	if ($bedEnd >= ${$RefID_r}{$nm}{"ex_starts"}[$i]) {
		if (${$RefID_r}{$nm}{"strand"} eq "+") {
...
foreach my$chr (@{$chromOrder_r}) {
	if (exists $interval3{$chr}) {
		foreach my$start ( sort{$a<=>$b}(keys%{ $interval3{$chr} }) ) { 
			print BED ${$chromName_r}{"bed"}{$chr}."\t".$start."\t".$interval3{$chr}{$start}{"end"}.$interval3{$chr}{$start}{"info"}."\n";
			}
		}
	}
close BED;
...
open (my$fh0, ">", "$bedFile\_0Chr.bed")  || die "cannot create $bedFile\_0Chr.bed\n";
open (my$fhW, ">", "$bedFile\_wChr.bed")  || die "cannot create $bedFile\_wChr.bed\n";
if (@{$chromOrder_r}) {
	foreach my$chr (@{$chromOrder_r}) {
