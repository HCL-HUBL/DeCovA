...
while (my$line=<BED>) {
	...
	if (($line =~ /^\w+\t\d+\t\d+/)&&($line !~ /^#/)) {
		...
		if ($tab[1] < $tab[2]) {
			...
			$bedLines[$j]{"Chrom"} = $chr; $bedLines[$j]{"Start"} = ($tab[1]+1); $bedLines[$j]{"End"} = $tab[2];
			if ($tab[3] && $tab[3] !~ /^\s*$/) { $bedLines[$j]{"Infos"} = $tab[3]; }

			$Chr{$i} = $chr; $Start{$i} = ($tab[1]+1); $End{$i} = $tab[2];		##1-based
	...
	while (my$line = <$fh>) {
		...
		if ($pos == $Starts[$c2]) {
			foreach my$end (@{ $Intervals2{$chr}{$Starts[$c2]} }) {
				foreach (@{$Files_r}) {
					${$min}{$_}{$chr}{$Starts[$c2]}{$end} = $tab[${$smplIdx_r}{$_}];
					${$max}{$_}{$chr}{$Starts[$c2]}{$end} = $tab[${$smplIdx_r}{$_}];
					}
				}
			}
		while ( ($pos >= $Starts[$c2]) && ($c2 < $#Starts) ) {
			foreach my$end (@{ $Intervals2{$chr}{$Starts[$c2]} }) {
				if ($pos < $end) {
					fillVals ($Files_r,$smplIdx_r,$threshold,$chr,$pos,$Starts[$c2],$end,$cov,$notCov,$noCovStart,$noCovEnd,\@tab,$min,$max,$vals);
					}
				elsif ($pos == $end) {
					fillVals ($Files_r,$smplIdx_r,$threshold,$chr,$pos,$Starts[$c2],$end,$cov,$notCov,$noCovStart,$noCovEnd,\@tab,$min,$max,$vals);
					computeVals ($Files_r,$mergeBed_r,$threshold,$chr,$pos,$Starts[$c2],$end,$cov,$sum,$mean,$median,$TotBases,$TotByCov,$TotByDepth,$vals);
					}
			 	}
			$c2++;
			}
...
if ($meanTotBases) { $meanTotBases /= scalar@{$Files_r}; }
...
foreach (@{$Files_r}) {
	foreach(@{$covFields_r}) {
		if ($_ eq "cov") { print $fh1 "\t%>=$threshold"."X"; }
		else { print $fh1 "\t$_"; }
		}
	}
print $fh1 "\n";
...
for (my$i=0;$i<scalar@allLines;$i++) {
	...
	if (exists $Chr{$i}) {
		foreach my$file (@{$Files_r}) {
			...
			foreach(@{$covFields_r}) {
				if ($_ eq "min") { print $fh1 "\t".${$min}{$file}{$Chr{$i}}{$Start{$i}}{$End{$i}}; }
				elsif ($_ eq "max") { print $fh1 "\t".${$max}{$file}{$Chr{$i}}{$Start{$i}}{$End{$i}}; }
				elsif ($_ eq "tot") { print $fh1 "\t".${$sum}{$file}{$Chr{$i}}{$Start{$i}}{$End{$i}}; }
				elsif ($_ eq "mean") { print $fh1 "\t".sprintf("%.1f",${$mean}{$file}{$Chr{$i}}{$Start{$i}}{$End{$i}}); }
				elsif ($_ eq "median") { print $fh1 "\t".sprintf("%.1f",${$median}{$file}{$Chr{$i}}{$Start{$i}}{$End{$i}}); }
				elsif ($_ eq "cov") { print $fh1 "\t".100*(sprintf("%.3f", ${$cov}{$file}{$Chr{$i}}{$Start{$i}}{$End{$i}}))."%"; }
				}
